...
silence_model=None, silence_model_path='', batch_size=64, device="cpu", n_classes = None):
    ...
    predict_dir = os.path.join(ConfigManager().get_base_path(), 'predictions/{output_file}/')
    get_confusion_matrix(predict_dir, final_model, unknown_model, silence_model, test_transform, device, n_classes)
