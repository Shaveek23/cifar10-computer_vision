...
def save_files(validation, test, suffix):
    pd.Series(validation).to_csv(TXT_DIRECTORY+f"validation_silence{suffix}.txt",
                                 header=None, index=None, sep='\n', mode='a')
    pd.Series(test).to_csv(TXT_DIRECTORY+"test_silence{suffix}.txt",
                           header=None, index=None, sep='\n', mode='a')

def generate_silence(size,suffix='', validation_size=0.1, test_size=0.1):
    """
    need to provide the size that's possible to be divided by 6 
    """
    files_names = []
    isExist = os.path.exists(SILENCE_DIRECTORY+suffix+"\\")
    if not isExist:
        os.makedirs(SILENCE_DIRECTORY+suffix+"\\")
    ...
    for file_nr, file in enumerate(glob(BACKGROUND_NOISE_DIRECTORY + "*.wav")):
        for i in range(per_file_size):
            ...
            torchaudio.save(os.path.join(SILENCE_DIRECTORY+suffix+"\\",
    ...
    save_files(files_names[validation_indexes], files_names[test_indexes],suffix)

if __name__ == '__main__':
    generate_silence(int(sys.argv[1]),sys.argv[2])
