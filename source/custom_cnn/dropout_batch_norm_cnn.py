...
class DBN_cnn(nn.Module):

    def __init__(self, n_blocks=3, conv_k=None, drop_out=None, fc_k=128, fc_drop_out= 0.5,
        is_fc_drop_out=True, is_batch_norm=True, is_drop_out=True, n_classes=10, n_chans=3):
        ...
        self.fc1 = nn.Linear(in_features=self.conv_k[-1]*4*4, out_features=self.fc1_k)
        
        self.fc_batch_norm = nn.BatchNorm1d(num_features=self.fc1_k, momentum=0.99, eps=0.001)
...
class DBN_Block(nn.Module):
    def __init__(self, conv_k, drop_out, n_chans, is_batch_norm=True, is_drop_out=True):
        ...
        self.batch_norm1 = nn.BatchNorm2d(num_features=self.conv1_k, momentum=0.99, eps=0.001)

        self.conv2 = nn.Conv2d(in_channels=self.conv1_k, out_channels=self.conv2_k, kernel_size=(3,3), padding=1)
        self.batch_norm2 = nn.BatchNorm2d(num_features=self.conv2_k, momentum=0.99, eps=0.001)
